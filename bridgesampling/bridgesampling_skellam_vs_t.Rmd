...
} else { stanfit_tdist <- readRDS("output/stanfit_tdist.rds")}
```
...
} else { stanfit_skellam <- readRDS("output/stanfit_skellam.rds")}
```
...
```{r}
set.seed(2)
bridge_tdist <- bridge_sampler(stanfit_tdist, silent = T)
bridge_tdist
```

```{r}
set.seed(2)
bridge_skellam <- bridge_sampler(stanfit_skellam, silent = T)
bridge_skellam
```
