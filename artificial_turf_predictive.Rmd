...
fullrun <- 0
if(fullrun) {
  epl_np_67 <- readRDS("FITS/fit_epl_np_67.rds")
  a_sims <- extract(epl_np_67, "a")
    library(arrayhelpers)
  # a_sims is 3000 x 135 x 23 teams = 9M numbers
  ma_sims <- array2df(a_sims$a, levels = list(NA, NA, NA), label.x = "value")
  
  ma_sims <- as.data.table(ma_sims)
...
gp1 <- MakeTimeSeriesPlot(ma_sims, title = "T-distribution base model: no pooling", id_lut, subset = c("Ajax", "Feyenoord")) + ylim(c(-3,3)) + theme_bw()

rm(ma_sims)

fullrun <- 0
if(fullrun) {
  epl_67 <- readRDS("FITS/fitX_67.rds")
...
gp2 <- MakeTimeSeriesPlot(ma_sims, title = "T-distribution base model: partial pooling", id_lut, subset = c("Ajax", "Feyenoord")) + ylim(c(-3,3)) + theme_bw()
